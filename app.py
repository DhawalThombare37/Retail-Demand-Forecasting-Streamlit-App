...
if uploaded_file:
    ...
    # --- SAVE ORIGINAL TARGET BEFORE ANY TRANSFORMATION ---
    if 'Demand Forecast' not in df_full.columns:
        st.error("❌ 'Demand Forecast' column missing in CSV!")
    else:
        y_original = df_full['Demand Forecast'].copy()  # for test MAPE later
        ...
        sequence_length = joblib.load("sequence_length.pkl")  # used in transformer
        ...
        y_true = y_original.iloc[test_indices].values  # use original target column
